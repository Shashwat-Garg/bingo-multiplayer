...
var server = http.createServer(function (req, res) {
	...
	res.end(
		"Wrong path, try going to http://shashwat-garg.github.io/bingo-multiplayer"
	);
});
...
socket.on("connect", function (client) {
	...
	client.on("userNameInput", function (data, callback) {
		...
		if (!data || validator.validate(data, schema).errors.length > 0) {
			...
			data[tags.USERNAME] == data[tags.ADD_TO_ROOM] &&
			rooms[data[tags.ADD_TO_ROOM]]
		) {
			var errorObj = {
				success: false,
			...
			if (data[tags.ADD_TO_ROOM] && rooms[data[tags.ADD_TO_ROOM]]) {
				if (
					rooms[data[tags.ADD_TO_ROOM]][tags.GAME_STATUS] ==
					status.NOT_STARTED
				) {
					var errorObj = {
						success: false,
					...
					if (
						rooms[data[tags.ADD_TO_ROOM]][tags.PLAYERS].length == 2
					) {
						var errorObj = {
							success: false,
	...
	client.on("addThisUserToRoom", function (data, callback) {
		...
		if (!data || validator.validate(data, schema).errors.length > 0) {
			...
			callback(errorObj);
			console.log(
				"Attempt to breach! Disconnecting socket " +
					client[tags.SOCKET_ID]
			);
			if (client[tags.SOCKET_ID] in socket.sockets.connected) {
				client.disconnect(true);
			}
		} else if (data[tags.ADD_TO_ROOM] && data[tags.ADD_TO_ROOM] in rooms) {
			rooms[data[tags.ADD_TO_ROOM]][tags.PLAYERS].push(
				data[tags.USERNAME]
			);
	...
	client.on("matrixElementInput", function (obj) {
		...
		if (!obj || validator.validate(obj, schema).errors.length > 0) {
			...
			!(tags.USERNAME in client) ||
			client[tags.USERNAME] != obj[tags.USERNAME]
		) {
			console.log(
				"Attempt to breach! Disconnecting socket " +
					client[tags.SOCKET_ID]
			...
			rooms[obj[tags.ADD_TO_ROOM]][obj[tags.USERNAME]][
				tags.USER_TABLE
			].push(obj[tags.ELEMENT]);
		} else {
			rooms[obj[tags.USERNAME]][obj[tags.USERNAME]][tags.USER_TABLE].push(
				obj[tags.ELEMENT]
			);
	...
	client.on("convertUserTableTo2D", function (obj, callback) {
		...
		if (!obj || validator.validate(obj, schema).errors.length > 0) {
			...
			var temp =
				rooms[obj[tags.USERNAME]][obj[tags.USERNAME]][tags.USER_TABLE];
			rooms[obj[tags.USERNAME]][obj[tags.USERNAME]][
				tags.USER_TABLE
			] = new Array(5);
			for (var i = 0; i < 5; i++) {
				rooms[obj[tags.USERNAME]][obj[tags.USERNAME]][tags.USER_TABLE][
	...
	client.on("modifyHost", function (data) {
		...
		if (!data || validator.validate(data, schema).errors.length > 0) {
			...
			delete rooms[data[tags.USERNAME]][data[tags.USERNAME]][
				tags.ARR_BOT
			];
			delete rooms[data[tags.USERNAME]][data[tags.USERNAME]][
				tags.BOT_TABLE
			...
			rooms[data[tags.USERNAME]][tags.GAME_STATUS] =
				status.WAITING_FOR_PLAYERS;
	...
	client.on("userSelectedElement", function (obj) {
		...
		if (!obj || validator.validate(obj, schema).errors.length > 0) {
			...
			if (
				rooms[obj[tags.ADD_TO_ROOM]][tags.LEFT_ARRAY].indexOf(
					obj[tags.ELEMENT]
				) < 0
			) {
				var errorObj = {
				...
				asyncLoop(
					rooms[obj[tags.ADD_TO_ROOM]][tags.PLAYERS],
					function (key, next) {
						modUserArr(
							obj[tags.ADD_TO_ROOM],
							key,
							tags.ARR_USER,
							tags.USER_TABLE,
							obj[tags.ELEMENT]
						);
						rooms[obj[tags.ADD_TO_ROOM]][key][
							tags.COUNT_USER
						][0] = checkBingo(
							rooms[obj[tags.ADD_TO_ROOM]][key][tags.ARR_USER]
						);
						var retObj = {
							success: true,
							error: "",
						...
						if (temp[tags.COUNT_USER][0] == 5) {
							if (
								...
								rooms[obj[tags.ADD_TO_ROOM]][tags.GAME_STATUS] =
									status.GAME_ENDED;
								winnerObj.winner = key;
							}
							socket
								.in(obj[tags.ADD_TO_ROOM])
								.emit("gameHasEnded", winnerObj);
						}
						retObj[tags.COUNT_USER] = temp[tags.COUNT_USER][0];
						retObj[tags.ELEMENT] = obj[tags.ELEMENT];
						socket
							.to(rooms[key][tags.SOCKET_ID])
							.emit("sendData", retObj);
						// console.log(rooms);
						next();
					},
					function (err) {
			...
			rooms[obj[tags.USERNAME]][tags.LEFT_ARRAY].indexOf(
				obj[tags.ELEMENT]
			) < 0
		) {
			...
			rooms[obj[tags.USERNAME]][obj[tags.USERNAME]][tags.COUNT_BOT]
		) {
			modUserArr(
				obj[tags.USERNAME],
				obj[tags.USERNAME],
			...
			rooms[obj[tags.USERNAME]][obj[tags.USERNAME]][
				...
				rooms[obj[tags.USERNAME]][obj[tags.USERNAME]][tags.ARR_USER]
			);
			rooms[obj[tags.USERNAME]][obj[tags.USERNAME]][
				tags.COUNT_BOT
			][0] = checkBingo(
				rooms[obj[tags.USERNAME]][obj[tags.USERNAME]][tags.ARR_BOT]
			);
			...
			setTimeout(function () {
				...
				if (
					temp[tags.COUNT_USER][0] == 5 &&
					temp[tags.COUNT_BOT][0] == 5
				) {
					winnerObj.draw = true;
				} else if (temp[tags.COUNT_USER][0] == 5) {
					winnerObj.winner = obj[tags.USERNAME];
				...
				if (
					...
					if (
						rooms[obj[tags.USERNAME]][tags.GAME_STATUS] !=
						status.GAME_ENDED
					) {
						rooms[obj[tags.USERNAME]][tags.GAME_STATUS] =
							status.GAME_ENDED;
						socket
							.in(obj[tags.USERNAME])
							.emit("gameHasEnded", winnerObj);
					}
				}
				retObj[tags.COUNT_USER] =
					rooms[obj[tags.USERNAME]][obj[tags.USERNAME]][
						tags.COUNT_USER
					][0];
			...
			// console.log(rooms);
			asyncLoop(
				rooms[obj[tags.USERNAME]][tags.PLAYERS],
				function (key, next) {
					modUserArr(
						obj[tags.USERNAME],
						key,
					...
					rooms[obj[tags.USERNAME]][key][
						tags.COUNT_USER
					][0] = checkBingo(
						rooms[obj[tags.USERNAME]][key][tags.ARR_USER]
					);
					var retObj = {
						success: true,
						error: "",
	...
	client.on("disconnect", function () {
		if (tags.USERNAME in client) {
			if (client[tags.USERNAME] == client[tags.CURR_ROOM]) {
				...
				asyncLoop(
					rooms[client[tags.USERNAME]][tags.PLAYERS],
					function (key, next) {
						delete rooms[key];
						next();
					},
				...
				var index = rooms[client[tags.CURR_ROOM]][tags.PLAYERS].indexOf(
					client[tags.USERNAME]
				);
				if (index !== -1)
					rooms[client[tags.CURR_ROOM]][tags.PLAYERS].splice(
						index,
						1
					);
				var obj = {
